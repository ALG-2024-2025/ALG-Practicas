...
def ordena(
    ...
    Args:
        sequencia (list): Secuencia a ordenar.
...
def quicksort(secuencia: list, key_function: callable, reverse: bool):
    ...
    def _insertion_sort(low, high):
        for i in range(low + 1, high + 1):
            ...
            key_value = key_function(key_item)
            j = i - 1
            # Insertion sort in ascending or descending order based on reverse flag
            if not reverse:
                while j >= low and key_function(secuencia[j]) > key_value:
    ...
    def _median_of_three(low, high):
        ...
        if (not reverse and key_function(secuencia[low]) > key_function(secuencia[mid])) or \
           (reverse and key_function(secuencia[low]) < key_function(secuencia[mid])):
            secuencia[low], secuencia[mid] = secuencia[mid], secuencia[low]
        if (not reverse and key_function(secuencia[low]) > key_function(secuencia[high])) or \
           (reverse and key_function(secuencia[low]) < key_function(secuencia[high])):
            secuencia[low], secuencia[high] = secuencia[high], secuencia[low]
        if (not reverse and key_function(secuencia[mid]) > key_function(secuencia[high])) or \
           (reverse and key_function(secuencia[mid]) < key_function(secuencia[high])):
    ...
    def _partition(low, high):
        ...
        for j in range(low, high):
            if (not reverse and key_function(secuencia[j]) <= pivot_key) or \
               (reverse and key_function(secuencia[j]) >= pivot_key):
                i += 1
